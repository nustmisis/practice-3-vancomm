...
выводит к какому типу относится данный гос номер, или возвращает 'Fail!', если
это не гос номер.

Упрощение: на ввод будут подаваться только Тип 1, Тип1А, Тип1Б, Тип 2, Тип 3,
Тип 4
...
plate_types = {
    "1А": re.compile(r"[авекмнорстух]\d{3}[авекмнорстух]{2} \d{2,3}"),
    "1Б": re.compile(r"[авекмнорстух]{2}\d{3} \d{2,3}"),
    "2": re.compile(r"[авекмнорстух]{2}\d{4} \d{2,3}"),
    "3": re.compile(r"\d{4}[авекмнорстух]{2} \d{2,3}"),
}


def get_plate_type(plate):
    for name, pattern in plate_types.items():
